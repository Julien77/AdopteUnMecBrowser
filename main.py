...
'''
#check only profile on the current view
elementsProfile = driver.find_elements_by_css_selector("h4 a")  
                        for elementProfile in elementsProfile:
                                elementProfile.send_keys(Keys.CONTROL + Keys.RETURN)
                                time.sleep(5)
                                driver.switch_to_window(driver.window_handles[1])
                                time.sleep(30)
                                driver.close()
...
'''
        strLocator.send_keys(commune)

        tribPageLink = driver.find_element_by_css_selector('table#example tbody tr.odd td:last-child')
        if(tribPageLink.text == "Rien trouvé - désolé"):
            log_errors_list.append('Warning ! website communes.actualitix.com (3rd try)\n')
            modifiedCommune = commune.split('-')[0]
            strLocator.clear()
            strLocator = driver.find_element_by_css_selector("input[type='search']")
            strLocator.send_keys(modifiedCommune)
            tribPageLink = driver.find_element_by_css_selector('table#example tbody tr.odd td:last-child')
            tribPageLink.click()
            urlFound = driver.current_url
            title = driver.find_element_by_css_selector('h1.name')
            newCommune.append((title.text.split(': ')[1])[:-6])
            driver.close()
            return urlFound
        else:
            tribPageLink.click()
